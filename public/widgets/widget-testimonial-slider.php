 ...
 * @uses _Widget
 */
class WPC_Shortcodes_Widget_Testimonial_Slider extends WP_Widget {
	function __construct() {

		$widget_ops = array( 'description' => __('Add testimonial slider to your widget area.') );
		parent::__construct( 'wc_shortcodes_testimonial_slider', __('Angie Makes - Testimonial Slider'), $widget_ops );
	}
	...
	function widget($args, $instance) {
		...
		if ( ! empty( $shortcode ) ) {
			...
			$shortcode = '[wc_testimonial_slider ' . $shortcode . '][/wc_testimonial_slider]';
		}
		else {
			$shortcode = '[wc_testimonial_slider][/wc_testimonial_slider]';
		}
	...
	function update( $new_instance, $old_instance ) {
		$new_instance = WPC_Shortcodes_Sanitize::testimonial_slider_attr_fix_bools( $new_instance );
		$instance = WPC_Shortcodes_Sanitize::testimonial_slider_attr( $new_instance );
	...
	function form( $instance ) {
		...
		$o = array_merge( WPC_Shortcodes_Vars::$attr->testimonial_slider, $instance );
		$o = WPC_Shortcodes_Sanitize::testimonial_slider_attr( $o );
		
		$post_types = WPC_Shortcodes_Widget_Options::post_types('wcs_slide');
		?>

		<div id="wc-shortcodes-testimonial-slider-widget-<?php echo $this->number; ?>" class="wc-shortcodes-testimonial-slider-widget wc-shortcodes-visual-manager wpc-ui-theme-override">
			<h3>Select Posts</h3>
			<div>
				<p>
			...
			<div>
				<p>
					...
					<select class="wc-shortcodes-widget-option" id="<?php echo $this->get_field_id('template'); ?>" name="<?php echo $this->get_field_name('template'); ?>">
						<?php foreach ( WPC_Shortcodes_Widget_Options::testimonial_slider_templates() as $key => $value ) : ?>
			...
			<div>
				<p>
					...
					<select class="wc-shortcodes-widget-option" id="<?php echo $this->get_field_id('slider_mode'); ?>" name="<?php echo $this->get_field_name('slider_mode'); ?>">
						<?php foreach ( WPC_Shortcodes_Widget_Options::testimonial_slider_modes() as $key => $value ) : ?>
		...
		<script type="text/javascript">
			...
			jQuery(document).ready(function($){
				$('#wc-shortcodes-testimonial-slider-widget-<?php echo $this->number; ?>').accordion({heightStyle: "content", collapsible: true}).wcPostsWidget();
			});
			/* ]]> */
		</script>
		<?php
